...
class Cache(object):
    ...
    def get_next(self, idx):
        if self.buf[idx+self.VALID_OFF]&self.NEXT_OK == 0:
            return -1
        next_idx_byte = self.buf[idx+self.NEXT_IDX_OFF:idx+self.NEXT_IDX_OFF+self.INDEX_LEN]
    ...
    def _write_inode(self, lastnode_idx, data_idx):
        ...
        curnode_idx_byte = curnode_idx.to_bytes(self.INDEX_LEN, self.BYTE_ORDER)
        lastnode_idx_byte = lastnode_idx.to_bytes(self.INDEX_LEN, self.BYTE_ORDER)
        ...
        while True:
            ...
            for key in self.task_heads.keys():
                ...
                if self.buf[idx]&self.USED == 0:
                    new_head = int.from_bytes(self.buf[idx+self.NEXT_IDX_OFF:idx+self.NEXT_IDX_OFF+self.INDEX_LEN], byteorder=self.BYTE_ORDER)
    ...
    def _allocate_data(self, ):
        ...
        while True:
            ...
            for i in range(self.size-self.DATA_LEN, self.data_head-1, -self.DATA_LEN):
                ...
                for ref in refs:
                    ref_data_idx = int.from_bytes(self.buf[ref+self.DATA_IDX_OFF:ref+self.DATA_IDX_OFF+self.INDEX_LEN], self.BYTE_ORDER)
                    if self.buf[ref+self.VALID_OFF]&self.DATA_OK != 0 and ref_data_idx == i:
                        free = False
                        break
                if free:
                    return i
    ...
    def debug_print(self, ):
        for i in range(0, self.inode_tail, self.INODE_LEN):
            print(self.buf[i],int.from_bytes(self.buf[i+1:i+5], self.BYTE_ORDER),int.from_bytes(self.buf[i+5:i+9], self.BYTE_ORDER),end=' | ')
