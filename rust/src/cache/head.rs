use std::{convert::TryInto, slice::from_raw_parts_mut, slice::from_raw_parts};
...
impl Head {
    ...
    pub fn set_end(&mut self, end: bool) {
        unsafe { self.ptr.copy_from((end as u32).to_be_bytes().as_ptr(), 4) };
    }
    ...
    fn get_end(&mut self) -> bool {
        let slice = unsafe { from_raw_parts(self.ptr, 4) };
